...
int usage(const char*const argv0, const int ret)
{
    std::cerr << "Usage: " << argv0 << "[options...] inputImage outputDir\n"
                 ...
                 "   --align-horizon                    Align horizon to the pixel grid to avoid a\n"
                 "                                       semi-transparent scanline at the horizon\n"
                 "                                       (adjusts the value of -b)\n"
                 "   --weigh-by-alpha                   Weigh supersampling samples by alpha instead\n"
                 "                                      of simply averaging\n"
                 ;
    return ret;
}
...
#define GO_TO_PARAM()                                                       \
    ...
    if(alignHorizonToPixelGrid)
    {
        ...
        const double jUnaligned = (sideBottomAngularShift - latitudeToAlign) / sideAngularHeight * sideHeight + sideHeight - 0.5;
        // For alignment the value of j corresponding to latitudeToAlign must be a half-integer, because it's right between two pixels
        const double jAligned = std::round(jUnaligned + 0.5) - 0.5;
        sideBottomAngularShift = latitudeToAlign - (sideHeight - 0.5 - jAligned) / sideHeight * sideAngularHeight;
    }

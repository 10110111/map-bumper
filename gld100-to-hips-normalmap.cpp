...
double fetch(std::vector<Tile> const& data, const ssize_t requestedX, const ssize_t requestedY, const ssize_t resolutionAtEquator)
{
    ...
    const auto y = std::clamp(requestedY, ssize_t(0), totalHeight-1);
...
double sample(std::vector<Tile> const& data, const double resolutionAtEquator,
...
Tile readTile(QString const& inDir, QString const& sector)
{
    Tile tile;
    tile.sector = sector;
    const auto filename = inDir + "/WAC_GLD100_" + sector + "_100M.IMG";
    ...
    tile.kmPerUnit = 1e-3 * sampleScalingFactor;
...
glm::dvec3 computeNormal(const double centerLon, const double centerLat,
                         ...
                         const double kmPerUnit, const double sphereRadius)
{
    using namespace glm;
    ...
    const double eastHeight = kmPerUnit*sample(heightMapTiles, resolutionAtEquator, eastLon, eastLat);
    const double eastRadius = sphereRadius + eastHeight;

    const double westHeight = kmPerUnit*sample(heightMapTiles, resolutionAtEquator, westLon, westLat);
    const double westRadius = sphereRadius + westHeight;

    const double northHeight = kmPerUnit*sample(heightMapTiles, resolutionAtEquator, northLon, northLat);
    const double northRadius = sphereRadius + northHeight;

    const double southHeight = kmPerUnit*sample(heightMapTiles, resolutionAtEquator, southLon, southLat);
...
void fillFace(const int order, const int pix, const std::vector<Tile>& heightMapTiles,
              ...
              const double kmPerUnit, const double sphereRadius, uint8_t* outData)
{
    ...
    for(int y = 0; y < HIPS_TILE_SIZE; ++y)
    {
        for(int x = 0; x < HIPS_TILE_SIZE; ++x)
        {
            ...
            const auto normal = computeNormal(longitude, latitude, heightMapTiles,
                                              resolutionAtEquator, kmPerUnit, sphereRadius);
...
#define GO_TO_PARAM()                                                       \
    ...
    {
        if(heightMapTiles[i].kmPerUnit != heightMapTiles[i-1].kmPerUnit)
        {
            throw std::runtime_error(QString(u8"Unit scaling factors of sectors %1 and %2 don't match: %3 km/unit vs %4 km/unit")
                                        .arg(heightMapTiles[i].sector).arg(heightMapTiles[i-1].sector)
                                        .arg(heightMapTiles[i].kmPerUnit).arg(heightMapTiles[i-1].kmPerUnit)
                                        .toStdString());
        }
    }
    const double kmPerUnit = heightMapTiles[0].kmPerUnit;
    for(unsigned i = 1; i < heightMapTiles.size(); ++i)
    {
    ...
    std::cerr << "Creating tiles of order " << orderMax << "...\n";
    {
                     ...
                     &numThreadsReportedFirstProgress,&itemsDone](const int pixMin, const int pixMax)
        {
            ...
            {
                fillFace(orderMax, pix, heightMapTiles, channelsPerPixel, resolutionAtEquator, kmPerUnit, sphereRadius, data.data());
